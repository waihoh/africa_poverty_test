...
def get_per_image_histograms(init_iter, batch_op, band_bin_edges):
    """
    :param init_iter: tf op, initializes the dataset iterator
    :param batch_op: dict, str => tf.Tensor
...
def split_nl_hist(image_hists, years):
    """
    :param image_hists: np.array, shape [nimages, C, nbins], last band is NIGHTLIGHTS
    :param years: np.array, shape [nimages]
    :return: image_hists_nl: np.array, shape [nimages, C+1, nbins], 2nd-to-last band is DMSP, last band is VIIRS
    """
    nimages, C, nbins = image_hists.shape
    image_hists_nl = np.zeros(shape=[nimages, C+1, nbins])
...
def plot_band_hists(band_hists, band_order, band_colors, bin_centers, xlabel, ylabel, title, yscale='linear'):
    """
    :param band_hists: np.array, shape [C, nbins], assumes DMSP and VIIRS each have their own band
    :param band_order: list of str
    :param band_colors: dict, band_name => matplotlib color
    :param bin_centers: np.array, shape [nbins]
    :param xlabel: str
    :param ylabel: str
    :param title: str
    :param yscale: str, one of ['linear', 'log']
    """
...
def plot_label_hist(labels, bin_edges, title, figsize=(10, 4)):
    """
    :param labels: np.array, shape [num_images]
    :param bin_edges: np.array, shape [num_label_bins + 1]
    :param title: str
