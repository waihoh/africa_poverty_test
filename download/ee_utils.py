...
def decode_qamask(img: ee.Image) -> ee.Image:
    ...
    cloud_shadow = qa.bitwiseAnd(8).eq(0)  # 0 = shadow, 1 = no shadow
    cloud_shadow = cloud_shadow.updateMask(cloud_shadow).rename(['pxqa_cloudshadow'])

    snow = qa.bitwiseAnd(16).eq(0)  # 0 = snow, 1 = no snow
...
def mask_qaclear(img: ee.Image) -> ee.Image:
    '''
    :param img: ee.Image
    :return:  ee.Image, input image with cloud-shadow, snow, cloud, and unclear pixels masked out
    '''

    qam = decode_qamask(img)
    cloudshadow_mask = qam.select(opt_selectors='pxqa_cloudshadow')
    snow_mask = qam.select(opt_selectors='pxqa_snow')
    cloud_mask = qam.select(opt_selectors='pxqa_cloud')
    return img.updateMask(cloudshadow_mask).updateMask(snow_mask).updateMask(cloud_mask)  # TODO: check updateMask
...
def composite_nl(year: int) -> ee.Image:
    ...
    :param year: int, start year of survey
    :return: ee.Image, contains a single band named 'NIGHTLIGHTS'
    '''

    if year <= 2011:
        img_col = ee.ImageCollection(args='NOAA/DMSP-OLS/CALIBRATED_LIGHTS_V4')
    else:
        img_col = ee.ImageCollection(args='NOAA/VIIRS/DNB/MONTHLY_V1/VCMSLCFG')

    start_date, end_date = surveyyear_to_range(year, nl=True)
    return img_col.filterDate(start=start_date, opt_end=end_date).median().select([0], ['NIGHTLIGHTS'])
...
def tfexporter(collection: ee.FeatureCollection, export: str, prefix:str,
    ...
    """
    Creates and starts a task to export a ee.FeatureCollection to a TFRecord file in Google Drive / Google Cloud Storage (GCS)
    GCS: gs://bucket/prefix/fname.tfrecord
    Drive: prefix/fname.tfrecord

    :param collection: ee.FeatureCollection
    :param export: str, 'drive' for Drive, 'gcs' for GCS
    :param prefix: str, folder name in Drive of GCS to export to, no trailing '/'
    :param fname: str, filename
    :param selectors: None of ee.List of str, names of properties to include in output, set to None to include all properties
    :param dropselectors: None or ee.List of str, names of properties to exclude
    :param bucket: None or str, name of GCS bucket, only used if export=='gcs'
    :return: task: ee.batch.Task
    """
    if dropselectors is not None:
        if selectors is None:
            selectors = collection.first().propertyName()
    ...
    if export == 'gcs':
        task = ee.batch.Export.table.toCloudStorage(
            ...
            description=fname, bucket=bucket,
...
def sample_patch(point: ee.Feature, patches_array: ee.Image, scale: Numeric) -> ee.Feature:
    '''
    Extracts an image patch at a specific point
    :param point: ee.Feature
    :param patches_array: ee.Image, Array Image
    :param scale: int or float, scale in meters of the projection to sample in
    :return: ee.Feature, 1 property per band from the input image
    '''
...
def get_array_patches(img: ee.Image,
                      ...
                      dropselectors: Optional[ee.List] = None, bucket: Optional[str] = None) -> ee.batch.Task:
    """
    Creates and starts a task to export square image patches in TFRecord format to Google Drive or Google Cloud Storage (GCS).
    The image patches are sampled from the given ee.Image at specific coordinates.
    :param img: ee.Image, image covering the entire reqion of interest
    :param scale: int or float, scale in meters of the projection to sample in
    :param ksize: int or float, radius of square image patch
    :param points: ee.FeatureCollection, coordinates from which to sample patches
    :param export: str, 'drive' for Google Drive, 'gcs' for GCS
    :param prefix: str, folder name in Drive of GCS to export to, no trailing "/"
    :param fname: str, filename for export
    :param selectors: None or ee.List, names of properties to include in output, set to None to include all properties
    :param dropselectors: None or ee.List, names of properties to exclude
    :param bucket: None or str, name of GCS bucket, only used if export == 'gcs'
    :return: ee.batch.Task
    """
    ...
    # ee.Image.sampleRegions() does not cut it for larger collections, using mapped sample instead
    samples = points.map(lambda pt: sample_patch(point=pt, patches_array=patches_array, scale=scale))
    ...
    return tfexporter(collection=samples, export=export, prefix=prefix,
                      fname=fname, selectors=selectors, dropselectors=dropselectors, bucket=bucket)


def wait_on_tasks(tasks: Mapping[Any, ee.batch.Task],
                  show_probar: bool = True,
                  poll_interval: int = 20) -> None:
    """
    Display a progress bar of task progress
    :param tasks: dict, maps task ID to a ee.batch.Task
    :param show_probar: bool, whether to display progress bar
    :param poll_interval: int, # of seconds between each refresh
    """
    ...
    while len(remaining_tasks) > 0:
        ...
        for taskID in remaining_tasks:
            ...
            if state in done_states:
                ...
                progbar.write(s=f'Task {taskID} finished in {elapsed_min} min with state: {state}')
            else:
...
class LandsatSR:
    ...
    def init_coll(self, name: str) -> ee.ImageCollection:
        """
        Creates a ee.ImageCollection containing images of desired points between the desired start and end dates
        :param name: str, name of collection
        :return: ee.ImageCollection
        """
        return (ee.ImageCollection(name).filterBounds(self.filterpoly).filterDate(start=self.start_date, opt_end=self.end_date))
    ...
    def rescale_l8(img: ee.Image) -> ee.Image:
        """
        :param img: ee.Image, Landsat 8 image, with bands already renamed by rename_18()
    ...
    def rescale_l57(img: ee.Image) -> ee.Image:
        """
        :param img: ee.Image, landsat 5/7 image, with bands already renamed by rename_157()
        :return: ee.Image, with bands rescaled
        """
